...
def _get_recipient(claim: HapClaim) -> dict:
    """Get recipient name and domain from a claim"""
    if claim.get("type") == "recipient_commitment":
        return claim.get("recipient", {})
    return claim.get("to", {})


def _get_method(claim: HapClaim) -> str:
    """Get method from a claim (or commitment for recipient_commitment)"""
    if claim.get("type") == "recipient_commitment":
        return claim.get("commitment", "")
    return claim.get("method", "")


def encode_compact(claim: HapClaim, signature: bytes) -> str:
    """
    ...
    """
    recipient = _get_recipient(claim)
    method = _get_method(claim)

    fields = [
        f"HAP{HAP_COMPACT_VERSION}",
        claim.get("id", ""),
        claim.get("type", ""),
        method,
        _encode_compact_field(recipient.get("name", "")),
        _encode_compact_field(recipient.get("domain", "")),
...
def decode_compact(compact: str) -> DecodedCompact:
    ...
    parts = compact.split(".")
    if len(parts) != 10:
        raise ValueError("Invalid HAP Compact format: expected 10 fields")

    version, hap_id, claim_type, method, encoded_name, domain, at_unix, exp_unix, iss, sig_b64 = parts

    if version != f"HAP{HAP_COMPACT_VERSION}":
    ...
    if claim_type == "recipient_commitment":
        claim: HapClaim = {
            **base_claim,
            ...
            "recipient": {"name": name, **({"domain": decoded_domain} if decoded_domain else {})},
            "commitment": method,
        }
    else:
        claim = {
...
def is_valid_compact(compact: str) -> bool:
    ...
    """
    return bool(HAP_COMPACT_REGEX.match(compact))


def build_compact_payload(claim: HapClaim) -> str:
    """
...
def sign_compact(claim: HapClaim, private_key: Ed25519PrivateKey) -> str:
    """
...
def verify_compact(compact: str, public_keys: list[HapJwk]) -> CompactVerificationResult:
    """
