...
public class Verifier : IDisposable
{
    ...
    public Verifier(HttpClient? httpClient = null)
    {
        if (httpClient != null)
        {
    ...
    public async Task<WellKnown> FetchPublicKeysAsync(string issuerDomain, CancellationToken cancellationToken = default)
    {
        var url = $"https://{issuerDomain}/.well-known/hap.json";
        var response = await _httpClient.GetStringAsync(url, cancellationToken);
        return JsonSerializer.Deserialize<WellKnown>(response) ?? new WellKnown();
    }
    ...
    public async Task<SignatureVerificationResult> VerifySignatureAsync(string jws, string issuerDomain, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var claim = JsonSerializer.Deserialize<Claim>(payloadJson);
    ...
    public async Task<Claim?> VerifyClaimAsync(
        string hapId,
        string issuerDomain,
    ...
    public static bool IsClaimExpired(Claim claim)
    {
    ...
    public static bool IsClaimForRecipient(Claim claim, string recipientDomain)
    {
