...
export async function fetchPublicKeys(
  ...
  try {
    ...
    return (await response.json()) as WellKnown;
  } finally {
...
export async function verifySignature(
  ...
  try {
    ...
    const jwk = wellKnown.keys.find((k: Jwk) => k.kid === kid);
    if (!jwk) {
    ...
    // Decode the payload
    const claim = JSON.parse(new TextDecoder().decode(payload)) as Claim;
...
export async function verifyClaim(
  ...
  return "claim" in response ? response.claim : null;
}
